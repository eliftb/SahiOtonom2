...
def select_device(device='', batch_size=None):
    ...
    s = f'YOLOPv2 torch {torch.__version__}'
...
def lane_line_mask(ll):
    # ll: torch tensor, modelden çıkan çıktı

    # Genellikle ll shape: (batch, channels, height, width)
    # Eğer batch varsa al
    if ll.ndim == 4:
        ll = ll[0]

    # Eğer kanal varsa (örneğin 1 kanal), kanal boyutunu squeeze et
    if ll.shape[0] == 1:
        ll = ll.squeeze(0)

    # ll şimdi 2D (height, width) olmalı

    # Maskeyi 0.5 eşik değeri ile binarize et
    ll_mask = (ll > 0.5).cpu().numpy().astype(np.uint8)
