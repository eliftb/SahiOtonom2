...
class Yolo_subscriber(Node):
    ...
    def yolo_callback(self, data):
        ...
        for r in data.yolov8_inference:
            ...
            # Dikdörtgenin köşe noktalarını kullanarak merkez noktasını hesapla
            center_x = int((left + right) / 2)
            center_y = int((top + bottom) / 2)
       
             # Merkez noktaya kırmızı bir nokta çiz
            cv2.circle(img, (center_x, center_y), 5, (0, 0, 255), 1)
            
            yolo_subscriber.get_logger().info(f"{self.cnt} {class_name} : {top}, {left}, {bottom}, {right}: CENTERY-X: {center_y}, {center_x}")
