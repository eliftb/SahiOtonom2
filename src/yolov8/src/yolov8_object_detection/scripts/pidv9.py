...
class WallFollower(Node):
    ...
    def display_info(self):
        ...
        if self.last_detected_sign is not None:
            sign_text = f"Tabela: {self.last_detected_sign}"
        ...
        if self.parking_mode:
            cv2.putText(display_img, "Arac Modu: Park Modu", (10, 250), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 0), 2)
        elif self.stop_sign_detected or self.waiting or self.durak_mode:
            cv2.putText(display_img, "Arac Modu: Durak Modu", (10, 250), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 0), 2)
        else:
            cv2.putText(display_img, "Arac Modu: Sürüş Modu", (10, 250), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 0), 2)

        # Display the image in a window
        cv2.imshow('Telemetri', display_img)
        cv2.waitKey(1)
