    ...
    def camera_callback(self, data):
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                # Crop and show the detected object in a separate window
                cropped_img = img[int(b[1]):int(b[3]), int(b[0]):int(b[2])]
                cv2.imshow(f"Detected Object - {self.model.names[int(c)]}", cropped_img)

            self.get_logger().info(f"{self.yolov8_inference}, center x,y: {merkez_x}, {merkez_y}")
